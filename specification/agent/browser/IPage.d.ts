...
export interface IPage extends ITypedEventEmitter<IPageEvents> {
    ...
    addNewDocumentScript(script: string, isolateFromWebPageEnvironment: boolean, callbackFns?: {
        [name: string]: (payload: string, frame: IFrame) => any | null;
    }, devtoolsSession?: IDevtoolsSession): Promise<{
        identifier: string;
    }>;
