...
class FramesManager extends eventUtils_1.TypedEventEmitter {
    ...
    constructor(page, devtoolsSession) {
        ...
        this.events.on(this.websocketSession, 'message-received', this.onWebsocketSessionMessageReceived);
    }
        ...
        if (callbacks) {
            ...
            for (const [name, onCallbackFn] of Object.entries(callbacks)) {
                ...
                if (existing.length === 0)
                    this.pageCallbacks.set(name, existing);
    ...
    async onWebsocketSessionMessageReceived(event) {
